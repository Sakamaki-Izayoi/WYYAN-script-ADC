...
namespace Jhin_As_The_Virtuoso {
	...
	class Jhin {
		...
		internal static void OnLoad(EventArgs args) {
			...
			if (IsChinese)
			{
				Game.PrintChat("最强戏命师　".ToHtml(25) + "\u5a1b&#27005;VIP脚本".ToHtml(Color.PowderBlue, FontStlye.Cite));
			}
			else
			{
		...
		private static void LoadMenu() {
			...
			Config.AddSeparator();
			Config.AddLabel("娛樂VIP脚本群：215226086").SetFontStyle(FS.Bold, DXColor.PapayaWhip);
